...
ax.plot(xg, line(xg, m_true, b_true), "k--", lw=1, label="true")
...
sigma_with_cov = np.sqrt(
    np.clip(xg * xg * var_m + var_b + 2.0 * xg * cov_mb, 0.0, np.inf)
)
sigma_indep = np.sqrt(
    np.clip(xg * xg * (float(m.stderr) ** 2) + float(b.stderr) ** 2, 0.0, np.inf)
)
