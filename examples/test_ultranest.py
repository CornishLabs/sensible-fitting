...
def main() -> None:
    ...
    # Fit curve plot using posterior mean
    ...
    if samples.ndim == 2 and samples.shape[0] > 0:
        ...
        mean = pts.mean(axis=0) if pts.ndim == 2 and pts.shape[0] > 0 else None

    fig, ax = plt.subplots()
    ax.errorbar(x, y, yerr=yerr, fmt="x", label="data")
    ax.plot(x, y_clean, "k:", alpha=0.6, label="true")

    if mean is not None:
        y_fit = model(x, float(mean[0]), float(mean[1]), float(mean[2]))
        ax.plot(x, y_fit, "-", label="posterior mean")
        ax.set_title(
            f"posterior mean: loc={mean[0]:.3f}, amp={mean[1]:.3f}, width={mean[2]:.3f}"
        )
    else:
